export const TEAM = [
  { id: 'james', name: 'James',  role: 'Lead Manager',      initials: 'J', hue: 210 },
  { id: 'ben',   name: 'Ben',    role: 'Acquisition Agent', initials: 'B', hue: 195 },
  { id: 'davis', name: 'Davis',  role: 'Acquisition Agent', initials: 'D', hue: 38  },
];
...
export const APPT_TYPE = Object.fromEntries(APPT_TYPES.map(t => [t.id, t]));
...
export function buildWeek(base = new Date()) {
...
export function seedAppointments(weekKeys) {
  const rows = [
  ...
  return rows.map(([dayIdx, personId, start, end, type, title, addr]) => ({
    ...
    personId, start, end, type, title, address: addr,
  }));
}

export function busyFor(appts, dayKey, personId) {
  return appts.filter(a => a.dayKey === dayKey && a.personId === personId)
              .map(a => [a.start, a.end]);
}

export function findFreeSlots(appts, dayKey, personIds, durationMin, slotStep = 30) {
...
export function detectConflicts(appts, draft) {

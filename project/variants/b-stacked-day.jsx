// ───────────────────────────────────────────────────────────────
// VARIANT B — "Stacked Day"
// Classic day column with all three agents overlaid, each with their
// color. Best for spotting overlap visually. Hour grid, vertical.
// ───────────────────────────────────────────────────────────────

function StackedDayApp() {
  ...
  const [visible, setVisible] = React.useState(() => new Set(TEAM.map(p => p.id)));
  ...
  const HOUR_PX = 96; // 96px per hour
  const totalHeight = ((DAY_END - DAY_START) / 60) * HOUR_PX;

  const openNew = (dayK, startMin) => setModal({
    open: true,
    draft: { dayKey: dayK, start: startMin, end: startMin + 60, personId: TEAM[0].id, type: 'walkthrough' },
  });
  ...
  // Column layout — lay agents' appointments side-by-side within the day column.
  const columnsFor = (personIds) => {
    // Each personId gets its own column within the day so overlap reads clearly.
    const visiblePeople = TEAM.filter(p => personIds.has(p.id));
    return visiblePeople;
  };
  const cols = columnsFor(visible);
        ...
        <main style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              }} style={dayNavBtnB}><Icon name="chevron-left" size={18} /></button>
              ...
              }} style={dayNavBtnB}><Icon name="chevron-right" size={18} /></button>
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '20px 32px 64px' }}>
            <div style={{
              background: '#fff', borderRadius: 16, boxShadow: '0 2px 6px rgba(17,17,17,0.06)',
              overflow: 'hidden',
            }}>
              {/* Column header row */}
...
function DayColumn({ person, hourPx, totalHeight, appts, onEdit, onEmpty }) {
  ...
  return (
    <div ref={ref} onClick={onBg} style={{
      flex: 1, position: 'relative', height: totalHeight, borderLeft: '1px solid #F1F1F1',
      cursor: 'crosshair',
    }}>
      {/* hour lines */}
      {Array.from({ length: (DAY_END - DAY_START) / 60 }, (_, i) => (
...
Object.assign(window, { StackedDayApp });

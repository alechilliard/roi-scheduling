// ───────────────────────────────────────────────────────────────
// VARIANT C — "Find a Time"
// Agent-picker + duration → instant free-slot suggestions.
// The booking-first flow: tell the app who + how long + when-ish,
// it surfaces free windows to pick from. Best for "book a client fast".
// ───────────────────────────────────────────────────────────────

function FindTimeApp() {
  const week = React.useMemo(() => buildWeek(new Date('2026-04-22')), []);
  const todayKey = week[2].key;
  const [appts, setAppts] = React.useState(() => seedAppointments(week.map(w => w.key)));
  ...
  const [modal, setModal] = React.useState({ open: false, draft: null });
  const [hoveredSlot, setHoveredSlot] = React.useState(null);
  const [mode, setMode] = React.useState('all');
  const selectedIds = Array.from(selected);

  const resultsAll = React.useMemo(() => findFreeSlots(appts, dayKey, selectedIds, duration),
    [appts, dayKey, selectedIds.join(','), duration]);

  const anySlots = React.useMemo(() => {
    const out = [];
    ...
    <div style={{ minHeight: '100vh', background: '#F1F1F1', fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#111' }}>
      <header style={{
        ...
        <div style={{
          ...
          boxShadow: '0 20px 60px rgba(0, 32, 76, 0.25)', padding: 24,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 0.8fr auto', gap: 20, alignItems: 'flex-end' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 6, padding: 4, background: '#F1F1F1', borderRadius: 10 }}>
                <button onClick={() => setMode('all')} style={toggleBtnC(mode === 'all')}>All free</button>
                <button onClick={() => setMode('any')} style={toggleBtnC(mode === 'any')}>Any free</button>
              </div>
            </div>
            ...
            {week.map(d => {
              ...
              return (
                <button key={d.key} onClick={() => setDayKey(d.key)} style={{
                  ...
                  <div style={{ fontSize: 10, letterSpacing: '0.14em', fontWeight: 600, textTransform: 'uppercase', color: active ? '#0054D4' : '#6B6B6B' }}>{d.dow}{d.key === todayKey ? ' · today' : ''}</div>
      ...
      <main style={{ maxWidth: 1200, margin: '32px auto 0', padding: '0 32px 80px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 6px rgba(17,17,17,0.06)' }}>
          <TimelineRibbon appts={appts.filter(a => a.dayKey === dayKey && selected.has(a.personId))}
            people={TEAM.filter(p => selected.has(p.id))}
            windows={activeWindows} duration={duration}
            hovered={hoveredSlot} setHovered={setHoveredSlot} />
        </div>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12, marginTop: 12 }}>
            ...
            {activeWindows.slice(0, 12).map((w, i) => {
              ...
              const freePeople = TEAM.filter(p => free.includes(p.id));
              return (
                <button key={i}
          ...
          {windows.map((w, i) => {
            ...
            return (
              <div key={i}
                ...
                <div style={{ fontSize: 11, letterSpacing: '0.04em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>FREE · {fmtRange(w.start, w.end)}</div>
              </div>
            );
          })}
        </div>
            ...
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
              ...
              <div style={{ flex: 1, height: 36, position: 'relative', background: '#F7F8FA', borderRadius: 8 }}>
                {mine.map(a => {
                  ...
                  const left = ((a.start - DAY_START) / 60) * (HOUR_PX - 110/((DAY_END - DAY_START)/60));
                  // Simpler: compute as proportion of the remaining width
...
Object.assign(window, { FindTimeApp });

// ───────────────────────────────────────────────────────────────
// VARIANT A — "Team Rows"
// Horizontal timeline: one row per teammate, appointment blocks laid
// across time. Best scannable overview + drag-to-create.
// ───────────────────────────────────────────────────────────────
const { useState: useStateA, useMemo: useMemoA, useEffect: useEffectA, useRef: useRefA } = React;

function TeamRowsApp() {
  const week = useMemoA(() => buildWeek(new Date('2026-04-22')), []);
  const todayKey = week[2].key;
  const [dayKey, setDayKey] = useStateA(todayKey);
  const [appts, setAppts] = useStateA(() => seedAppointments(week.map(w => w.key)));
  const [modal, setModal] = useStateA({ open: false, draft: null });

  const dayAppts = appts.filter(a => a.dayKey === dayKey);
  const dayObj = week.find(w => w.key === dayKey);

  // Timeline sizing — each 30 min = 60px. 8am→7pm = 22 slots.
  const SLOT_PX = 72;
  const slots = [];
  ...
  const openNew = (personId, start) => setModal({
    open: true,
    draft: { dayKey, personId, start, end: start + 60, type: 'walkthrough' },
  });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F8FA', fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#111' }}>
      {/* Top nav — blue gradient hero bar */}
      <header style={{
        background: 'linear-gradient(135deg, #009EE9 0%, #0054D4 100%)',
        color: '#fff', padding: '18px 32px 22px',
      }}>
        ...
        <div style={{ maxWidth: 1440, margin: '28px auto 0', display: 'flex', alignItems: 'flex-end', gap: 32, justifyContent: 'space-between' }}>
          <div>
            ...
            }}>{dayObj.date.toLocaleDateString('en-US', { weekday: 'long' })}, <span style={{ color: '#F7A521' }}>{dayObj.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}.</span></h1>
      ...
      <div style={{
        ...
        padding: '16px 32px', position: 'sticky', top: 0, zIndex: 20,
      }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 14 }}>
          <button onClick={() => {}} style={dayNavBtn}><Icon name="chevron-left" size={18} /></button>
          ...
          <button onClick={() => {}} style={dayNavBtn}><Icon name="chevron-right" size={18} /></button>
      ...
      <main style={{ maxWidth: 1440, margin: '0 auto', padding: '24px 32px 80px' }}>
        <div style={{
          background: '#fff', borderRadius: 20, boxShadow: '0 2px 6px rgba(17,17,17,0.06)',
          overflow: 'hidden',
        }}>
          {/* Time header */}
              ...
              <div style={{ width: totalWidth, display: 'flex', height: 44, alignItems: 'flex-end' }}>
                {slots.map((t, i) => (
                  <div key={t} style={{
                    ...
                    paddingLeft: 8, paddingBottom: 8, fontSize: 11, fontWeight: 600, color: t % 60 === 0 ? '#3A3A3A' : '#B5B5B5',
              ...
              <div key={p.id} style={{ display: 'flex', borderBottom: pi === TEAM.length - 1 ? 'none' : '1px solid #F1F1F1' }}>
                ...
                <div style={{ flex: 1, overflowX: 'auto' }}>
                  <TimelineRow slots={slots} slotPx={SLOT_PX} totalWidth={totalWidth}
                    appts={mine} onEdit={openEdit}
                    onEmptyClick={(t) => openNew(p.id, t)} />
                </div>
              </div>
            );
          })}
...
function TimelineRow({ slots, slotPx, totalWidth, appts, onEdit, onEmptyClick }) {
  ...
  return (
    <div ref={rowRef} onClick={onBgClick} style={{
      width: totalWidth, minHeight: 96, position: 'relative', cursor: 'crosshair',
    }}>
      {/* Grid */}
      {slots.map((t, i) => (
        <div key={t} style={{
...
Object.assign(window, { TeamRowsApp });

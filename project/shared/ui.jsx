// Small shared UI primitives: buttons, avatars, logo, panel shells.
// Branded; intentionally flat/no framework.

const { useState, useEffect, useRef, useMemo } = React;

// ─────────────────── Button ───────────────────
function RoiButton({ children, variant = 'primary', size = 'md', onClick, icon, disabled, style = {}, type = 'button' }) {
  const bases = {
...
function Eyebrow({ children, color = '#6B6B6B', style }) {
  return <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 11, fontWeight: 400, letterSpacing: '0.24em', textTransform: 'uppercase', color, ...style }}>{children}</div>;
}

// ─────────────────── Avatar ───────────────────
function Avatar({ person, size = 32, ring, style = {} }) {
  ...
  return (
    <div title={person.name} style={{
      ...
      boxShadow: ring ? `0 0 0 2px #fff, 0 0 0 4px ${ring}` : 'none', ...style,
    }}>{person.initials}</div>
  );
}
function AvatarStack({ people, size = 28, overlap = 8 }) {
  return (
...
// ─────────────────── Logo ───────────────────
function RoiLogo({ color = '#fff', size = 18 }) {
  // Mini mark + wordmark set in brand display italic. Uses type to avoid
  // loading the png asset at small sizes and keep it sharp.
  return (
...
// Abstract roof + floor — stylized house (matches brand mark feel).
function RoiIconMark({ color = '#fff', size = 22 }) {
  return (
...
// ─────────────────── Icon (tiny inline set) ───────────────────
// Using inline SVG because Lucide CDN would be overkill here and the brand
// calls for 2px stroke, rounded caps.
...
Object.assign(window, {
  RoiButton, Eyebrow, Avatar, AvatarStack, RoiLogo, RoiIconMark, Icon,
});
